...
import { removeFolder } from '../../redux/reducers/menu';

const Menu = ({ menuItems }) => {
    const dispatch = useDispatch();

    const deleteFolder = useCallback((id, name) => {
        if (window.confirm(`Are you sure you want to delete ${name} folder? `)) {
            ...
            <ul className="menu-items">
                {menuItems.map(item => {
                    return <li className="menu-item" key={item.id}>
                        ...
                        {item.name}
                        <img onClick={() => deleteFolder(item.id, item.name)} src={deleteIcon} alt="delete-icon" />
                    </li>
                })}
            </ul>
            <AddItem />
