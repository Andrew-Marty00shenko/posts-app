...
import { addFolder } from '../../redux/reducers/menu';

const AddItem = () => {
    const colors = useSelector(state => state.menu.colors, shallowEqual);
    ...
    const openMenu = () => {
        setVisibleAddBlock(!visibleAddBlock)
    };
    ...
    const onAddFolder = () => {
        if (selectedColor === null) {
            alert("You don'n choose a color");
            return;
        }
        if (value === "") {
            alert("You should enter the folder name")
            return;
        }
        dispatch(addFolder(value, selectedColor));
        setValue("");
        closeMenu();
    };

    const selectColor = useCallback(name => {
        setSelectedColor(name);
    }, []);

    return (
        <div className="add-item">
            <div className="add-item__menu" onClick={openMenu}>
                <img src={plus} alt="plus-icon" />
                <span> Add folder</span>
            </div>
            {visibleAddBlock && <div className="add-toggle">
                <svg onClick={closeMenu} width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                />
                <div className="add-toggle__colors">
                    {
                        colors.map(c => {
                            return <li key={c.id}>
                                <div onClick={() => selectColor(c.name)}
                                    className={`${selectedColor === c.name ? `colors  ${c.name} active` : `colors ${c.name}`}`}>
                ...
                <button onClick={onAddFolder} className="add-toggle__btn">Add</button>
            </div>}
        </div>
    )
}
